...
@Database(
    ...
    exportSchema = false, // Aurora: disabled schema export (no migration schemas needed for fresh fork)
    autoMigrations = [
        // Aurora: auto-migrations disabled (schema files not included in fork)
        // Existing users will use destructive migration (fresh DB on update)
    ]
)
@TypeConverters(
    AppConverter::class,
